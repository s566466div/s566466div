### 👋 Hi, I’m Divya Bathala (@s566466div)

🎓 **Graduate Student | MS – Applied Computer Science**  
📍 Northwest Missouri State University | Class of 2025  
📫 Email: bathaladivya59@gmail.com | 📍 Mckinney, TX  
🔗 [LinkedIn](https://www.linkedin.com/in/divya-bathala)

---

💡 **Who I Am**  
A passionate **DevOps & Frontend Engineer** with 5+ years of hands-on experience in building CI/CD pipelines, automating cloud infrastructure, and developing scalable web applications. My work blends technical depth with system reliability and intuitive user experiences.

---

🛠️ **Core Skills & Tools**

**DevOps & Cloud**  
- Azure (AKS, App Services, Synapse) | AWS (EC2, RDS, IAM) | Terraform | Docker | Kubernetes | GitHub Actions | Jenkins  
- CI/CD (Azure DevOps YAML, GitHub Actions, Jenkins) | Containerization | Monitoring (Prometheus, Grafana, Azure Monitor)  

**Frontend & Backend**  
- React.js, JavaScript, HTML5, CSS3  
- Spring Boot, REST APIs, SQL, Firebase, MongoDB  

**Languages & Scripting**  
- Python, Java, Bash, PowerShell, SQL  

**Data & Analytics**  
- Power BI, Excel (Automation), SQL Server, Azure Synapse, Data Modeling  

---

🚀 **Highlighted Projects**

📘 **EduLearn – Cloud-Native LMS**  
`React | Spring Boot | Docker | Terraform | Netlify | Render`  
Built and deployed a full-stack LMS with CI/CD, containerized backend, and scalable infrastructure-as-code practices.

🤖 **Generative AI Chatbot**  
`LangChain | Hugging Face | Pinecone | Streamlit | GitHub Actions`  
Context-aware chatbot using RAG, deployed with CI/CD on Streamlit Cloud.

📊 **Power BI Titanic Survival Dashboard**  
Created advanced DAX measures and published an interactive dashboard with Git versioning.

📱 **LifeSaver – Organ Donation Android App**  
`Java | Firebase | Google Maps API`  
Developed an Android app to connect organ donors and recipients with real-time data and geolocation support.

🌐 **MongoDB Restaurant Finder**  
Built CRUD-based NoSQL web app with CLI automations and query optimizations.

---

🧠 **Currently Exploring**  
- Advanced cloud infrastructure design  
- Kubernetes production workflows  
- React performance optimization  
- Open-source contributions in DevOps/Automation

---

💼 **Open To**  
- Frontend Development Roles  
- DevOps & Cloud Engineering Opportunities  
- Full-Stack Development & Automation Projects  
- Open Source & Collaboration

---

✨ **Fun Fact**  
I thrive at the intersection of tech and creativity—automating backend systems while building beautiful, scalable frontends that deliver impact.

---

📍 Let’s Connect!  
[LinkedIn](https://www.linkedin.com/in/divya-bathala) | 📧 bathaladivya59@gmail.com

